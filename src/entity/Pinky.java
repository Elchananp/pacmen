...
//    Stack<String> pathStack = new Stack<>();
    ...
    public Pinky(GamePanel gamePanel, TileManager tileManager, Bfs bfs, Player player) {
...
//        exit = true;
    ...
    public void move() {
        ...
        if (direction.equals("up") && !tiles[indexTile].isBlocked && !tiles[indexTile].isBlockedForPinky) {
            y -= speed;

        } else if (direction.equals("down") && !tiles[indexTile].isBlocked && !tiles[indexTile].isBlockedForPinky) {
            y += speed;

        } else if (direction.equals("right") && !tiles[indexTile].isBlocked && !tiles[indexTile].isBlockedForPinky) {
            x += speed;

        } else if (direction.equals("left") && !tiles[indexTile].isBlocked && !tiles[indexTile].isBlockedForPinky) {
            x -= speed;
        }
    }
...
//        this.y = 375;
    ...
    public void getPath() {
        ...
        int[] PinkyPosition = {y / tileSize, x / tileSize};

        System.out.println("Player Position: " + Arrays.toString(playerPosition));
        System.out.println("Pinky Position: " + Arrays.toString(PinkyPosition));
//        System.out.println("Current Tile Map:");
//        debugMap(tileManager.getMap());
//        Stack<String> pathStack = new Stack<>();

//        bfs.blockAndFindNewPath(tileManager.getMap(), PinkyPosition, playerPosition);
        bfs.findShortestPath(tileManager.getMap(), PinkyPosition, playerPosition, true);
...
//            case "up" -> {
//                x1 = (y1 - (tileSize * 2)) - 1;
//                y1 = (player.x / tileSize);
//                return new int[]{x1, y1};
//            }
//            case "down" -> {
//                System.out.println("fasefefgegegeafeafgeafafaffaergfrgf");
//                x1 = (y1 * 2);
//                y1 = (player.x);
//                return new int[]{x1, y1};
//            }
...
//    }
    ...
    private int[] target() {
        int tileSize = gamePanel.tileSize;
        int x = player.x / tileSize; // חישוב מיקום X באריחים
        int y = player.y / tileSize; // חישוב מיקום Y באריחים

        switch (player.direction) {
            case "up" -> {
                System.out.println("row: " + (y - 2) + "col: " + (x));
                return new int[]{y - 2, x}; // שתי יחידות למעלה
            }
            case "down" -> {
                System.out.println("row: " + (y + 2) + "col: " + (x));
                return new int[]{y + 2, x}; // שתי יחידות למטה
            }
            case "right" -> {
                System.out.println("row: " + y + "col: " + (x + 2));
                return new int[]{y,x + 2}; // שתי יחידות ימינה

            }
            case "left" -> {
                System.out.println("fasefefgegegeafeafgeafafaffaergfrgf");
                System.out.println("row: " + y + "col: " + (x - 2));
                return new int[]{y,x - 2}; // שתי יחידות שמאלה
            }
            default -> {
                // ערך ברירת מחדל
                return new int[]{1, 1};
    ...
    public boolean hasPath(String nextStep) {
        ...
        if (!nextStep.equals("def") && index >= 0) {
            ...
            // בדיקת חסימה
            if (currentTile != null && !currentTile.isBlocked && !currentTile.isBlockedForPinky ) {
                return true;
            }
        }
        return false;
...
//      changeDirection(nextStep);
    ...
    public void getImage() {
        try {
            image = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/image/ghost-pinky-down.png")));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
